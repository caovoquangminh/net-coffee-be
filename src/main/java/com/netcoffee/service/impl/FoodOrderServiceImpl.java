...
public class FoodOrderServiceImpl implements FoodOrderService {
    ...
    public OrderResponse createOrder(Long userId, CreateOrderRequest request) {
        ...
        for (CreateOrderRequest.OrderItemRequest itemReq : request.getItems()) {
            ...
            // Kiểm tra tồn kho
            List<TInventoryItemEntity> invItems = inventoryItemRepository.findByMenuItemId(menuItem.getId());
            for (TInventoryItemEntity inv : invItems) {
                if (inv.getCurrentStock().compareTo(BigDecimal.valueOf(itemReq.getQuantity())) < 0) {
            ...
            if (itemReq.getAddonIds() != null && !itemReq.getAddonIds().isEmpty()) {
                List<TMenuItemAddonEntity> selectedAddons = menuItemAddonRepository
                        .findAllById(itemReq.getAddonIds());
        ...
        FoodOrderPaymentEnum paymentMethod = request.getPaymentMethod() != null
                ? request.getPaymentMethod() : FoodOrderPaymentEnum.CASH;
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            if (itemReq.getAddonIds() != null && !itemReq.getAddonIds().isEmpty()) {
                List<TMenuItemAddonEntity> selectedAddons = menuItemAddonRepository
                        .findAllById(itemReq.getAddonIds());
                for (TMenuItemAddonEntity addon : selectedAddons) {
                    foodOrderItemAddonRepository.save(TFoodOrderItemAddonEntity.builder()
    ...
    @Override
    @Transactional
    public OrderResponse updateStatus(Long orderId, OrderStatusEnum status, Long confirmedByUserId) {
    ...
    public OrderResponse cancelOrder(Long orderId, String reason, Long cancelledByUserId) {
        ...
        if (order.getStatus() == OrderStatusEnum.DONE || order.getStatus() == OrderStatusEnum.CANCELLED) {
    ...
    private void validateStock(List<TFoodOrderItemEntity> orderItems) {
        for (TFoodOrderItemEntity item : orderItems) {
            List<TInventoryItemEntity> invItems = inventoryItemRepository.findByMenuItemId(item.getItemId());
            for (TInventoryItemEntity inv : invItems) {
                ...
                if (inv.getCurrentStock().compareTo(BigDecimal.valueOf(item.getQuantity())) < 0) {
                    throw new IllegalStateException(
                            "Không thể giao đơn: '" + inv.getName() + "' trong kho chỉ còn "
                                    + inv.getCurrentStock() + " " + inv.getUnit());
                }
            }
        }
    }

    private void deductInventory(List<TFoodOrderItemEntity> orderItems, Long performedBy, Long orderId) {
        for (TFoodOrderItemEntity item : orderItems) {
            List<TInventoryItemEntity> invItems = inventoryItemRepository.findByMenuItemId(item.getItemId());
            for (TInventoryItemEntity inv : invItems) {
                ...
                inventoryTransactionRepository.save(TInventoryTransactionEntity.builder()
                ...
                if (isOutOfStock && inv.getMenuItemId() != null) {
                    menuItemRepository.findById(inv.getMenuItemId()).ifPresent(mi -> {
                        mi.setIsAvailable(false);
                        menuItemRepository.save(mi);
                    });
                }
                BigDecimal threshold = inv.getMinStock().compareTo(BigDecimal.TEN) > 0
                        ? inv.getMinStock() : BigDecimal.TEN;
                if (isOutOfStock || newStock.compareTo(threshold) < 0) {
                    messagingTemplate.convertAndSend("/topic/admin/inventory/low-stock", Map.of(
                            "inventoryItemId", inv.getId(),
    ...
    private String buildVietQrUrl(BigDecimal amount, String description) {
        String encodedName = URLEncoder.encode(vietQrAccountName, StandardCharsets.UTF_8);
        String encodedDesc = URLEncoder.encode(description, StandardCharsets.UTF_8);
        return String.format(
                "https://img.vietqr.io/image/%s-%s-compact2.png?amount=%s&addInfo=%s&accountName=%s",
                vietQrBankBin, vietQrAccountNumber, amount.toPlainString(), encodedDesc, encodedName);
    }

    private OrderResponse toResponse(TFoodOrderEntity order, List<TFoodOrderItemEntity> items, String qrImageUrl) {
        String confirmedByName = null;
        if (order.getConfirmedBy() != null) {
            confirmedByName = userRepository.findById(order.getConfirmedBy())
        ...
        List<OrderResponse.OrderItemResponse> itemResponses = items.stream()
                .map(i -> {
                    List<OrderResponse.AddonResponse> addonResponses = foodOrderItemAddonRepository
                            .findByOrderItemId(i.getId()).stream()
                            .map(a -> OrderResponse.AddonResponse.builder()
                    ...
                    try { itemName = menuItemService.findById(i.getItemId()).getName(); } catch (Exception ignored) {}
                    ...
                    return OrderResponse.OrderItemResponse.builder()
                            ...
                            .subtotal(i.getUnitPrice().add(addonTotal).multiply(BigDecimal.valueOf(i.getQuantity())))
