    ...
    private InventoryTransactionResponse toTxResponse(TInventoryTransactionEntity tx) {
        String itemName =
                inventoryItemRepository
                        .findById(tx.getInventoryItemId())
                        .map(TInventoryItemEntity::getName)
                        .orElse(null);
        String itemUnit =
                inventoryItemRepository
                        .findById(tx.getInventoryItemId())
                        .map(TInventoryItemEntity::getUnit)
                        .orElse(null);
        String supplierName = null;
        if (tx.getSupplierId() != null) {
            supplierName =
                    supplierRepository
                            .findById(tx.getSupplierId())
                            .map(s -> s.getName())
                            .orElse(null);
        }
        String performedByName =
                userRepository
                        .findById(tx.getPerformedBy())
